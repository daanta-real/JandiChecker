...
public class GithubMap {
	...
	public static Map<String, Object> getGithubMapInfo(String id) throws Exception {
		...
		while(it.hasNext()) {
			...
			// 정리
			count++;
			$.pn("찾아낸 날짜(" + count + "번째): " + sdf.format(cal.getTime()) + " > " + map.get(k));
		}
		...
		for(int i = commitTFs.length - 30; i < commitTFs.length; i++) {
			recentTotal++;
	...
	public static String getGithubInfoString(String name, String id) throws Exception {
		...
		sb.append("___***");
		sb.append(name);
		sb.append(" 님의 최근 커밋 근황***___\n");
		sb.append("최근 30일 간의 1일 1커밋률: ");
		float count = (int) map.get("recentCount");
